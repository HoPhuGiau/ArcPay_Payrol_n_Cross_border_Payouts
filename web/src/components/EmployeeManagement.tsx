...
export function EmployeeManagement() {
    ...
    // Placeholder data for demo/not connected state
    const employeeList = isConnected && companyId ? (employees as any[]) || [] : [
        { wallet: '0x0000...0000', basePay: BigInt(5000000000), payToken: '0x36...', active: true },
        { wallet: '0x0000...0000', basePay: BigInt(3000000000), payToken: '0x36...', active: false },
    ];
            ...
            {isLocked && (
                <div className="absolute inset-0 z-10 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-sm flex items-center justify-center">
                    <div className="text-center p-6 bg-white dark:bg-zinc-800 rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-700">
                        <p className="text-zinc-900 dark:text-white font-semibold mb-2">Connect Your Wallet</p>
            ...
            <div className={`transition-opacity duration-300 ${isLocked ? 'opacity-40 pointer-events-none select-none' : 'opacity-100'}`}>
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="bg-blue-100 dark:bg-blue-900/30 p-2 rounded-lg">
                        <svg className="w-6 h-6 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                    </div>
                </div>
                                    ...
                                    <tr key={idx} className="group hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-colors">
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="font-mono text-zinc-600 dark:text-zinc-300">{emp.wallet.slice(0, 6)}...{emp.wallet.slice(-4)}</span>
                                            </div>
                                        </td>
                                        <td className="py-4 px-4 font-medium">
                                            {formatAmount(emp.basePay)} <span className="text-zinc-500 font-normal text-xs">{emp.payToken === '0x3600000000000000000000000000000000000000' ? 'USDC' : 'EURC'}</span>
                                        </td>
                                        <td className="py-4 px-4">
